...
const MyBookings = () => {
  ...
  const d = {
    cancelledAt: null,
    confirmedAt: "2026-07-12T01:14:03.004Z",
    createdAt: "2026-07-12T01:14:03.004Z",
    customerInfo: {
      email: "omaryasir5712@gmail.com",
      name: "omer yasir",
      phone: "+249118988953",
      specialRequests: "",
    },
    ...
    paidAt: null,
    paymentDetails: null,
    pricing: { subtotal: 1500, tax: 150, total: 1650 },
    status: "confirmed",
    tour: null,
    travelers: 1,
    _id: "yOKj4SD8i3MNaSlI3siddA",
  };
  ...
  const canCancelBooking = (booking) => {
    ...
    return (
      ...
      (booking.status === "paid" && daysDifference > 3)
    );
  };
  ...
  if (!user) {
    return (
      <div
        className={`min-h-screen py-12 ${isRTL ? "font-arabic arabic-text" : "font-english"}`}
      >
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4">
          <div className="flex flex-wrap gap-2">
            {["all", "confirmed", "paid", "cancelled", "completed"].map(
              (status) => (
                <button
                  key={status}
        ...
        <div className="space-y-6">
          {filteredBookings.length === 0 ? (
            ...
            filteredBookings.map((booking) => {
              if (!booking.tour) return null; // Skip rendering if tour data is missing
              const StatusIcon = getStatusInfo(booking.status).icon;
              const statusConfig = getStatusInfo(booking.status);
              const paymentConfig = getPaymentStatusInfo(
                      ...
                      <div className="flex-1">
                        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between mb-4">
                          ...
                          <div className="mt-4 lg:mt-0 flex flex-col items-end gap-2">
                            ...
                            <span
                              className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium ${paymentConfig.color}`}
                            >
                              {paymentConfig.text}
                            </span>
                          </div>
                    ...
                    <div className="flex flex-wrap justify-between items-center text-sm text-gray-600 dark:text-gray-400">
                      ...
                      {booking.cancelledAt && (
                        <span className="text-red-600 dark:text-red-400">
                          {isRTL ? "تم الإلغاء في:" : "Cancelled on:"}{" "}
